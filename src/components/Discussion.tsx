...
import { canReview, isSuspended } from "@/lib/permissions";
...
// composer. Both callers pass the settings of the event that owns the subject,
// which decide whether the thread is open and whether a verified email is
// needed to take part.
export default async function Discussion({
  ...
  const scoreById = new Map(tallies.map((t) => [t.targetId, t._sum.value ?? 0]));
  const voteById = new Map(myVotes.map((v) => [v.targetId, v.value]));

  // Now that comments can be voted on, show the thread best-first. Ties fall
  // back to oldest-first so the ordering is stable between renders.
  ...
  const canVote = !!user && !suspended && !needsVerify && commentsEnabled;
    ...
    <section className="discussion">
      ...
      {!commentsEnabled ? (
        <p className="muted notice-closed">
          🔒 The host has closed the discussion here. Existing comments stay
          visible.
        </p>
      ) : !user ? (
        <p className="muted">
