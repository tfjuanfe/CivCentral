...
// Up/downvote control for a single comment. The score shown after a cast comes
// from the server's own tally rather than from local arithmetic, so two people
// voting at once can't drift apart.
export default function CommentVotes({
  ...
  async function cast(value: 1 | -1) {
    if (!canVote) {
      // No reason given means "not logged in" — send them somewhere useful
